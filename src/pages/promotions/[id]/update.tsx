...
import { updatePromotion, getPromotion } from "@/api/promotions";
...
export default function AddProduct({ user, router, promotion, categories, products }: Props) {
    const [datas, setDatas] = React.useState<Promotion.Options>({ ...promotion })
    ...
    const update = async (e: any) => {
        ...
        const { errors } = await updatePromotion(promotion, config)
        if (errors.message)
            setError(errors)
    ...
    return (
        <Layout user={user} router={router} title={`Modifier ${promotion?.code} - Promotions`}>
            <form
                onSubmit={update}
                method="post"
